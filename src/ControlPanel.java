...
/**
 * ControlPanel Class - Panel kontrol modern dengan dropdown
 * UPDATED: UI/UX yang lebih clean dan intuitive
 */
public class ControlPanel extends JPanel {
    ...
    /**
     * Header panel dengan title dan new maze button
     * ⭐ PERUBAHAN DI SINI - Action listener yang lebih informatif
     */
    private JPanel createHeaderPanel() {
        ...
        // ⭐⭐⭐ BAGIAN YANG BERUBAH - Action listener baru ⭐⭐⭐
        newMazeBtn.addActionListener(e -> {
            ...
            if (maze.getStart() != null && maze.getEnd() != null) {
                int distance =
                        Math.abs(maze.getStart().getRow() - maze.getEnd().getRow()) +
                                Math.abs(maze.getStart().getCol() - maze.getEnd().getCol());


                resultArea.setText(
                        "✓ New maze generated!\n\n" +
                                "  Start: (" + maze.getStart().getRow() + ", " +
                                maze.getStart().getCol() + ")\n" +
                                "  End: (" + maze.getEnd().getRow() + ", " +
                                maze.getEnd().getCol() + ")\n" +
    ...
    private JPanel createActionButtonsPanel() {
        ...
        JButton compareBtn = createModernButton("Compare All Algorithms",
    ...
    private JPanel createLegendPanel() {
        ...
        panel.add(createCompactLegendItem(CellType.EMPTY.getName() + " (0)",
                CellType.EMPTY.getColor()));
        panel.add(createCompactLegendItem(CellType.GRASS.getName() + " (1)",
                CellType.GRASS.getColor()));
        panel.add(createCompactLegendItem(CellType.DIRT.getName() + " (5)",
                CellType.DIRT.getColor()));
        panel.add(createCompactLegendItem(CellType.WATER.getName() + " (10)",
                CellType.WATER.getColor()));
        ...
        for (PathResult result : allResults) {
            if (result.getPath() != null) {
                ...
                if (shortestPath == null ||
                        result.getPathLength() < shortestPath.getPathLength()) {
                    shortestPath = result;
                }
                if (fastest == null ||
                        result.getExecutionTime() < fastest.getExecutionTime()) {
                    fastest = result;
                }
            }
        }
